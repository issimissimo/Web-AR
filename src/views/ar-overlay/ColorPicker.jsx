...
const SaturationSlider = styled.div`
  position: relative;
  width: 100%;
  height: 8px;
  border-radius: 4px;
...
export default function ColorPicker(props) {
  ...
  // Calcola il colore esadecimale quando cambiano HSL
  createMemo(() => {
    const hexColor = hslToHex(hue(), saturation(), lightness());
    // Converte da "#ffffff" a numero esadecimale per ThreeJS
    const threeJSColor = parseInt(hexColor.replace('#', ''), 16);
    setColor(threeJSColor);
  });

  // Genera i background per saturation e lightness slider
  const saturationBackground = createMemo(() => 
  ...
  const handleStart = (e, sliderType, sliderRef) => {
    ...
    switch (sliderType) {
      case 'hue':
        ...
        break;
      case 'saturation':
        updateSliderValue(clientX, sliderRef, setSaturation, 100);
        break;
      case 'lightness':
  ...
  const handleMove = (e) => {
    ...
    switch (currentDragging) {
      case 'hue':
        ...
        break;
      case 'saturation':
        updateSliderValue(clientX, saturationRef, setSaturation, 100);
        break;
      case 'lightness':
  ...
  let hueRef, saturationRef, lightnessRef;
  ...
  return (
    <ColorPickerContainer>
      ...
      <SaturationSlider 
        ...
        onMouseDown={(e) => handleStart(e, 'saturation', saturationRef)}
        onTouchStart={(e) => handleStart(e, 'saturation', saturationRef)}
      >
        <SliderThumb
          style={{ left: `calc(${(saturation() / 100) * 100}% - 12px)` }}
          animate={{ scale: dragging() === 'saturation' ? 1.1 : 1 }}
          transition={{ duration: 0.1 }}
        />
      </SaturationSlider>
